...
RSpec.describe Student, type: :model do
  let (:user) { User.create email: "sam@test.com",  password: "123456", password_confirmation: "123456"
    ...
    Student.create name:"Elle Woods", notes: "okay", rank:"Very High", image:"https://i.insider.com/5f08ce4baee6a82702176d61?width=700", user_id: user.id
    scully= Student.create name:"Elle Woods", notes: "okay", rank:"Very High", image:"https://i.insider.com/5f08ce4baee6a82702176d61?width=700", user_id: user.id 
    expect(scully.errors[:name]).to_not be_empty
  end
  it "is not valid without notes" do
    scully= Student.create name:"Elle Woods", rank: "Alien", image: "https://asdf.jpg", user_id: user.id
    expect(scully.errors[:notes]).to_not be_empty
    end
  it "is not valid without user_id" do
    scully= Student.create name:"Elle Woods", rank: "Alien", image: "https://asdf.jpg"
  ...
  it "is not valid without image" do
    scully= Student.create name:"Elle Woods", rank: "Alien", user_id: user.id
